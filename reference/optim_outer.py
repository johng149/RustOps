...
def create_tensors(
    ...
    layer0_counts = net.layer_counts[0].clone().detach() # Counts used in optim_outer
    ...
    # Execute optim_outer (modifies net.layers[0] in place)
    net.optim_outer(deltas)

    # Capture state *after* optim_outer
    ...
    save_reference(middle_delta, dir, f"{name}_context_delta1")
    save_reference(root_delta, dir, f"{name}_context_delta2")
    save_reference(initial_layer1_weights, dir, f"{name}_context_layer1_weights_initial")
    save_reference(initial_layer2_weights, dir, f"{name}_context_layer2_weights_initial")
    save_reference(layer1_counts, dir, f"{name}_context_layer1_counts")
    save_reference(layer2_counts, dir, f"{name}_context_layer2_counts")
